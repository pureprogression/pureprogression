// Обработчик подписок Юкассы для WordPress
// Добавьте этот код в КОНЕЦ файла functions.php (БЕЗ открывающего тега <?php)
...
function handle_yookassa_subscription($payment_data) {
    ...
    $paymentStatus = isset($payment_data['status']) ? $payment_data['status'] : null;
    $paymentPaid = isset($payment_data['paid']) ? $payment_data['paid'] : false;
    ...
    error_log('YooKassa Subscription: Processing subscription for user ' . $userId . ', payment ' . $paymentId . ', status: ' . $paymentStatus);
    
    // Обрабатываем только успешные платежи
    // Примечание: Если платеж в статусе waiting_for_capture, он будет автоматически захвачен
    // в Next.js endpoints (activate-from-pending или activate-from-payment)
    // Webhook может прийти до capture, поэтому обрабатываем только succeeded
    if ($paymentStatus !== 'succeeded' || !$paymentPaid) {
        error_log('YooKassa Subscription: Payment not completed yet. Status: ' . $paymentStatus . ', Paid: ' . ($paymentPaid ? 'true' : 'false'));
        error_log('YooKassa Subscription: If status is waiting_for_capture, it will be captured automatically by Next.js');
        return false;
    }
