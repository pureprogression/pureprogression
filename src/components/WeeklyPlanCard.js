...
import { updateTaskStatus, addTaskComment, deleteTaskComment } from "@/lib/firebase";
...
export default function WeeklyPlanCard({ day, dayIndex, planId, userId, isCurrentDay = false }) {
  ...
  const handleAddComment = async (taskId) => {
    const commentValue = commentText[taskId] || '';
    if (!commentValue.trim()) {
      // Не сохраняем пустой комментарий
      return;
    }
    
    setIsSubmitting(true);
    const result = await addTaskComment(planId, dayIndex, taskId, commentValue, userId);
    if (result.success) {
      // Очищаем состояние комментария для этой задачи
  ...
  const handleDeleteComment = async (taskId) => {
    setIsSubmitting(true);
    const result = await deleteTaskComment(planId, dayIndex, taskId);
    if (result.success) {
      // Очищаем состояние комментария для этой задачи
      const newCommentText = { ...commentText };
      delete newCommentText[taskId];
      ...
      console.error('Ошибка при удалении комментария:', result.error);
      setIsSubmitting(false);
    }
  };
  ...
  // Вспомогательная функция для проверки наличия комментария
  const hasComment = (task) => {
    // Проверяем новый формат userComment
    // userComment должен существовать, не быть null/undefined, быть объектом и иметь текст
    if (task.userComment !== undefined && task.userComment !== null) {
      if (typeof task.userComment === 'object' && task.userComment.text) {
        const text = task.userComment.text;
        if (text && typeof text === 'string' && text.trim().length > 0) {
          return true;
        }
      }
      // Если userComment существует, но не является валидным объектом с текстом, комментария нет
      return false;
    }
    // Если userComment равен undefined или null, проверяем старый формат comments
    // НО НЕ показываем старые comments как userComment - они отдельные
    return false; // Старые comments не считаем за userComment
  };
  ...
  const renderTask = (task) => (
            ...
            {/* Кнопка добавить/редактировать комментарий */}
                ...
                onClick={() => {
                  ...
                  const existingComment = (task.userComment && task.userComment !== null && task.userComment.text) 
                                        ? task.userComment.text
                                        : (task.comments && task.comments.length > 0 ? task.comments[task.comments.length - 1].text : '');
                  // Всегда устанавливаем значение, даже если пустое (чтобы очистить старое значение)
          ...
          {/* Комментарий пользователя - кликабельный для редактирования */}
          {expandedTask !== task.id && hasComment(task) && (
            <div className="mt-2">
              <div 
                onClick={() => {
                  setExpandedTask(task.id);
                ...
                className="text-xs text-gray-400 bg-white/5 rounded p-2 cursor-pointer hover:bg-white/10 transition-colors"
              >
              ...
              <div className="flex gap-2">
                ...
                {hasComment(task) && (
                  <button
                    ...
                    disabled={isSubmitting}
                    className="px-3 py-1 bg-red-500/20 text-red-400 text-xs rounded-lg hover:bg-red-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
